/* Pure domain logic: no DOM, no localStorage, no globals, so every function
   is unit-testable without mocking. */
...
export function validateInkInput(input: InkInput): ValidationResult {
  ...
  else if (name.length > 80) errors.name = 'Nama tinta maksimal 80 karakter.';
  if (!brand) errors.brand = 'Merek wajib diisi.';
  else if (brand.length > 60) errors.brand = 'Merek maksimal 60 karakter.';
  ...
  if (notes.length > 600) errors.notes = 'Catatan maksimal 600 karakter.';
